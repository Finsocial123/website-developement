...
const MarketCapital = () => {
  ...
  const [trendingCoins, setTrendingCoins] = useState([]); // Initialize as an empty array
  ...
  useEffect(() => {
    fetch("http://saveai.tech/api/coins/coins/trending", {
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          ...
          setTrendingCoins([]); // Fallback to empty array if data is not an array
        }
      })
    ...
    fetch("http://saveai.tech/api/coins/coins/most-upvoted", {
      ...
      .then((data) => {
        // Ensure data is an array
        if (Array.isArray(data)) {
          setMostUpvotedCoins(data);
        } else {
          setMostUpvotedCoins([]); // Fallback to empty array if data is not an array
        }
      })
